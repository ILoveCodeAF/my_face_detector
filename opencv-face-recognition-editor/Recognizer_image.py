...
ap.add_argument("--detector", help="path to OpenCV's deep learning face detector",
                default="face_detection_model")
ap.add_argument("--embedding-model",  help="path to OpenCV's deep learning face embedding model",
                default='output/openface_nn4.small2.v1.t7')
ap.add_argument("--recognizer", help="path to model trained to recognize faces",
                default='output/recognizer.pickle')
ap.add_argument("--le", help="path to label encoder",
...
class RecognizerImage:
    ...
    def detect(self, isflip = False, flipCode = None):
        if self.image is None:
            print("No data image!")
            return None

        # if isflip:
        #     self.image = cv2.flip(self.image, flipCode)
...
# img = cv2.imread('images/hai_and_cuc.jpg', cv2.IMREAD_COLOR)
# temp = RecognizerImage()
# temp.setImage(img)
# temp.detect()
# cv2.waitKey(0)
# cv2.destroyAllWindows()